...
DEFAULT_TARGET = {"CrossAttention", "Attention", "GEGLU"}


def _find_modules(
    model,
    target_replace_module=DEFAULT_TARGET,
...
def inject_trainable_lora(
    model: nn.Module,
    target_replace_module: set[str] = DEFAULT_TARGET,
    r: int = 4,
...
def extract_lora_ups_down(model, target_replace_module=DEFAULT_TARGET):
...
def weight_apply_lora(model, loras, target_replace_module=DEFAULT_TARGET, alpha=1.0):
...
def monkeypatch_lora(model, loras, target_replace_module=DEFAULT_TARGET, r: int = 4):
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=nn.Linear
    ):
        weight = _child_module.weight
...
def monkeypatch_replace_lora(
    model, loras, target_replace_module=DEFAULT_TARGET, r: int = 4
):
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=LoraInjectedLinear
    ):
...
def monkeypatch_remove_lora(model, target_replace_module=DEFAULT_TARGET):
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=LoraInjectedLinear
    ):
        _source = (
